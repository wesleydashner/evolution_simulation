...
class Renderer:
    ...
    def __init__(self, block_width: int, board_size: int):
        pygame.init()
        self.__block_width = block_width
        self.__window_width = block_width * board_size
        self.__screen = pygame.display.set_mode((self.__window_width, self.__window_width))

    def render(self, board: List[List[Tuple[int, int, int]]]):
        self.__draw_grid(board, self.__block_width)
        pygame.display.update()

    def __draw_grid(self, board: List[List[Tuple[int, int, int]]], block_width: int):
        for i, row in enumerate(board):
            for j, column in enumerate(row):
                self.__draw_rectangle(i * block_width, j * block_width, block_width, board[j][i])

    # x, y are top left corner of rectangle
    def __draw_rectangle(self, x: int, y: int, width: int, color: Tuple[int, int, int]):
